...
/////////////////
    ...
    db.collection('sshuo').where({
      'shuoshuo_xingxi.bankuai':Number(this.data.bankuai),
      'shuoshuo_xingxi.jubao.1':db.command.lte(9),
      _time:db.command.gt(yizhou)
    }).orderBy(zuixinorzuire, 'desc').skip(head).limit(head+8).get().then(async(res)=>{
      if(res.data.length<(head+8)&&res.data.length<8){
        this.setData({
          loading:false
        })
      }
